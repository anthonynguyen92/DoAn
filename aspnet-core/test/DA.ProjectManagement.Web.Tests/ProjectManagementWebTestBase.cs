...
{
    public abstract class ProjectManagementWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static ProjectManagementWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(ProjectManagementWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<ProjectManagementDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<ProjectManagementDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<ProjectManagementDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<ProjectManagementDbContext, Task<T>> func)
        {
            T result;
